app.controller('custController', function($scope,Api){
	...
	Api.query({method:'getCustomers'},function(data){
...
app.controller('orderController', function($scope, Api){
	...
	Api.query({method:'getOrders'},function(data){
		$scope.orders = data;
	});
...
app.controller('productController', function($scope, Api){
	...
	Api.query({method:'getProducts'},function(data){
		$scope.products = data;
	});
...
app.controller('profileController', function($scope, $routeParams, Api){
	Api.get({method:'getOrders',id:$routeParams.id},function(data){
		$scope.profile = data;
		var total = 0.00;
...
app.controller('actionController', function($scope, $routeParams, $location, Api){
	...
	$scope.templateUrl = function() {
		return 'partials/action/' + $routeParams.page + '.php';
	}
	...
	$scope.delete = function() {
		...
		Api.delete({method:'delete',id:id,param:$routeParams.page},function(result){
	...
	if($routeParams.action != 'add' && $routeParams.id) {
		...
		Api.get({method:method,id:$routeParams.id}, function(data){
...
app.controller('actionOrderController', function($scope, $routeParams, Api, myService) {
	...
	$scope.save = function() {
	 ...
	 window.scope = $scope; // debug for crown browser
});
...
function save(Api, data, id) {
	if(data.action == 'add'){
		Api.save({method:'save'},data,function(result){
			if(result.$resolved == true) 
				window.location = '#/'+ data.page + 's';
		});
	}else if(data.action == 'update'){
		Api.update({method:'save',id:id},data,function(result){
			if(result.$resolved == true) 
				window.location = '#/'+ data.page + 's';
		});
	}
